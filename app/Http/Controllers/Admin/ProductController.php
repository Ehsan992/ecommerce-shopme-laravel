...
class ProductController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            $product="";
              $query=DB::table('products')->leftJoin('categories','products.category_id','categories.id')
                ...
                if ($request->new_added==1) {
                    $query->where('products.new_added',1);
                }
                if ($request->new_added==0) {
                    $query->where('products.new_added',0);
                }
            ...
            return DataTables::of($product)
                    ...
                    ->editColumn('featured',function($row){
                        if ($row->featured==1) {
                            return '<a href="#" data-id="'.$row->id.'" class="deactive_featurd"><span class="badge rounded-pill alert-success">Active</span> </a>';
                        }else{
                            return '<a href="#" data-id="'.$row->id.'" class="active_featurd"><span class="badge rounded-pill alert-danger">Diactive</span> </a>';
                        }
                    })
                    ->editColumn('today_deal',function($row){
                        if ($row->today_deal==1) {
                            return '<a href="#" data-id="'.$row->id.'" class="deactive_deal"><span class="badge rounded-pill alert-success">Active</span> </a>';
                        }else{
                            return '<a href="#" data-id="'.$row->id.'" class="active_deal"><span class="badge rounded-pill alert-danger">Diactive</span> </a>';
                        }
                    })
                    ->editColumn('status',function($row){
                        if ($row->status==1) {
                            return '<a href="#" data-id="'.$row->id.'" class="deactive_status"><span class="badge rounded-pill alert-success">Active</span> </a>';
                        }else{
                            return '<a href="#" data-id="'.$row->id.'" class="active_status"><span class="badge rounded-pill alert-danger">Diactive</span> </a>';
                        }
                    })
                    ->editColumn('new_added',function($row){
                        if ($row->new_added==1) {
                            return '<a href="#" data-id="'.$row->id.'" class="deactive_new_added"><span class="badge rounded-pill alert-success">Active</span> </a>';
                        }else{
                            return '<a href="#" data-id="'.$row->id.'" class="active_new_added"><span class="badge rounded-pill alert-danger">Diactive</span> </a>';
                        }
                    })
                    ->addColumn('action', function($row){
                        $actionbtn=' <a href="#" class="btn btn-sm font-sm  rounded btn-success  hover-up quick_view" id="'.$row->id.'" data-toggle="modal" data-target="#exampleModalCenter"><i class="fa fa-eye" style="font-size: 1.5em;color: white;"></i></a>
                        <a href="'.route('product.edit',[$row->id]).'" class="btn btn-sm font-sm rounded btn-brand  hover-up edit"><i class="material-icons md-edit" style="font-size: 1.5em;color: white;"></i></a> 
                        <a href="'.route('product.delete',[$row->id]).'" class="btn btn-sm font-sm btn-danger rounded hover-up" id="delete"><i class="material-icons md-delete_forever" style="font-size: 1.5em;color: white;"></i> 
                        </a>';

                       return $actionbtn;   
                    })
                    ->rawColumns(['action','category_name','subcategory_name','brand_name','thumbnail','featured','today_deal','status','new_added'])
                    ->make(true);       
        }
    ...
    public function store(Request $request)
    {
       ...
       $data['featured'] = $request->has('featured') ? $request->featured : 0;
        $data['today_deal'] = $request->has('today_deal') ? $request->today_deal : 0;
        $data['new_added'] = $request->has('new_added') ? $request->new_added : 0;
        $data['status'] = $request->has('status') ? $request->status : 0;
        $data['trendy'] = $request->has('trendy') ? $request->trendy : 0;
    ...
    public function update(Request $request)
    {
       ...
       $data['new_added']=$request->new_added;
    ...
    //not status
    public function not_new_added($id)
    {
        DB::table('products')->where('id',$id)->update(['new_added'=>0]);
        return response()->json('Product Deactive');
    }

    //active staus
    public function active_new_added($id)
    {
        DB::table('products')->where('id',$id)->update(['new_added'=>1]);
        return response()->json('Product Activated');
    }
