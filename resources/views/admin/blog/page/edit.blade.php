...
<main class="main-wrap">
  <section class="content-main">
    ...
    <div class="card mb-4">
      <div class="card-body">
        <form role="form" action="{{route('blog.page.update',$blog->id)}}" method="Post" enctype="multipart/form-data">
          ...
          <div class="card-body">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" name="blog_title" id="exampleInputEmail1" placeholder="Page Name" value="{{$blog->title}}">
            </div>
            <div class="form-group">
              <label class="form-label">Blog Image</label>
              <input class="form-control" type="file" name="thumbnail" required="">
              <input class="form-control" type="hidden" name="old_thumbnail" value="{{$blog->thumbnail}}" required="">
            </div>
            <div class="mb-4">
              <label class="form-label">Tags</label>
              <input type="text" class=form-control name="tags" value="{{$blog->tag}}" required="" data-role="tagsinput">
            </div>
            <div class="form-group">

              <label class="form-label">Blog Pgae Show</label>
              <select class="form-select" name="status">
                <option value="1" @if($blog->status==1) selected @endif>Yes</option>
                <option value="0" {{  ($blog->status==0) ? "selected" : "" }}>No</option>
              </select>
            </div>
            <div class="form-group">
              <label for="exampleInputPassword3">Blog Page Desciption</label>
              <textarea class="form-control textarea" rows="4" name="blog_description">{{$blog->description}}</textarea>
